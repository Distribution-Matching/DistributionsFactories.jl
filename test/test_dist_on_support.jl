    ...
    for (a, b) in [(-3.0, 2.0), (2.0, 7.0), (0.0, 10.0)]
        for α in [1.0, 2.0, 5.0]
            for β in [1.0, 3.0]
                ...
                μ̄ = a + w * μ_std
                σ̄² = w^2 * v_std
                d = dist_from_mean_var(Beta, μ̄, σ̄², a..b)
...
function test_support_uniform_scaled()
    d = dist_from_mean_var(Uniform, 0.0, 8.333333333333334, -5..5)
    ...
    for a in [1.0, 3.0, -2.0]
        for α in [2.0, 5.0]
            for θ in [1.0, 2.0]
                ...
                μ̄ = a + mean(d_std)
                σ̄² = var(d_std)
                d = dist_from_mean_var(Gamma, μ̄, σ̄², a..Inf)
                if !isapprox(mean(d), μ̄, rtol=1e-8) || !isapprox(var(d), σ̄², rtol=1e-8)
...
function test_support_gamma_flipped()
    for b in [5.0, 10.0, 0.0]
        ...
        μ̄ = b - mean(d_std)
        σ̄² = var(d_std)
        d = dist_from_mean_var(Gamma, μ̄, σ̄², -Inf..b)
        if !isapprox(mean(d), μ̄, rtol=1e-8) || !isapprox(var(d), σ̄², rtol=1e-8)
...
function test_support_binomial_shifted()
    for a in [5, 10, -3]
        ...
        μ̄ = a + mean(d_std)
        σ̄² = var(d_std)
        d = dist_from_mean_var(Binomial, μ̄, σ̄², a:b)
...
function test_support_discrete_uniform_shifted()
    d = dist_from_mean_var(DiscreteUniform, 12.5, 2.9166666666666665, 10:15)
...
function test_support_gamma_truncated()
    d = dist_from_mean_var(Gamma, 3.0, 1.0, 0..10)
...
function test_support_errors()
    ...
    try
        dist_from_mean_var(Beta, 0.5, 0.05, 0..Inf)
        return false
    catch e
    ...
    try
        dist_from_mean_var(Gamma, 5.0, 3.0, -Inf..Inf)
        return false
    catch e
        e isa ArgumentError || return false
    end
    ...
    try
        dist_from_mean_var(Gamma, 5.0, 3.0, -1..10)
        return false
    catch e
        e isa ArgumentError || return false
    end
